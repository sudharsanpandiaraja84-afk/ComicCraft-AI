...
from schemas import (
    ...
    QualityCheckReport, Chapter, CompleteStoryResponse,
    StoryImproveRequest, StoryContinueRequest, StoryRewriteRequest,
    StoryShortenRequest, StoryExpandRequest, ChapterGenerateRequest,
    SectionEditRequest, SectionEditResponse,
    RegenerateStoryRequest, CompareVersionsRequest, VersionComparisonResult,
    StorySceneExtractionRequest, StorySceneExtractionResponse,
    CharacterVisualProfile, LocationProfile, VisualScene, StoryPlotStructure
)
...
async def call_gemini_raw(
    ...
    if not active_key:
        raise ValueError("Google Gemini API key not found. Please configure GEMINI_API_KEY in .env or run in Demo Mode.")
        ...
        for model in FALLBACK_MODELS:
            ...
            try:
                ...
                if resp.status_code in [429, 404, 503, 500]:
                    err_msg = resp.text
                    logger.warning(f"Model {model} returned HTTP {resp.status_code}: {err_msg}. Falling back...")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if resp.status_code == 200:
                ...
                return {"valid": False, "message": f"API returned status {resp.status_code}: {resp.text}"}
    except Exception as e:
...
async def generate_characters(req: StoryGenerateRequest, blueprint: StoryBlueprint) -> List[StoryCharacter]:
...
async def generate_full_story(
    ...
    return {
        "title": data.get("title", blueprint.title),
...
async def continue_story(req: StoryContinueRequest) -> Dict[str, Any]:
    ...
    prompt = prompt_engine.build_continue_story_prompt(
        req.previous_story,
        req.continuation_prompt,
        req.target_length or "Medium"
    )
...
async def rewrite_story(req: StoryRewriteRequest) -> Dict[str, Any]:
    ...
    prompt = f"""Rewrite the following story with these new creative directions:
...
async def regenerate_story(req: RegenerateStoryRequest) -> CompleteStoryResponse:
    ...
    try:
        ...
        quality.strengths.insert(0, f"Genuinely divergent interpretation ({req.regeneration_option}) with zero paragraph duplication.")
...
async def compare_story_versions(req: CompareVersionsRequest) -> VersionComparisonResult:
    ...
    if req.demo_mode or not api_key:
        ...
        return VersionComparisonResult(
            ...
            characters_a=[c.name for c in va.story_data.characters] if va.story_data.characters else demo_comp["characters_a"],
            characters_b=[c.name for c in vb.story_data.characters] if vb.story_data.characters else demo_comp["characters_b"],
    ...
    try:
        ...
        return VersionComparisonResult(
            ...
            plot_differences=data.get("plot_differences", "Alternative narrative development and distinct investigative paths."),
            ending_differences=data.get("ending_differences", "Different climaxes and character fates."),
            tone_and_style_differences=data.get("tone_and_style_differences", "Different pacing and emotional beats."),
...
async def extract_story_scenes(req: StorySceneExtractionRequest) -> StorySceneExtractionResponse:
    ...
    if req.demo_mode or not api_key:
        ...
        demo_scenes_dict = demo_data.get_demo_story_scenes(
            ...
            target_count=req.target_scene_count or 5
        )
        return StorySceneExtractionResponse(
            ...
            locations=[LocationProfile(**l) for l in demo_scenes_dict["locations"]],
    ...
    try:
        ...
        for c in data.get("characters", []):
            if isinstance(c, dict):
                snippet = c.get("appearance_prompt_snippet") or f"{c.get('name', 'character')}, {c.get('clothing', '')}, {c.get('face_description', '')}"
                characters.append(CharacterVisualProfile(
        ...
        for l in data.get("locations", []):
            if isinstance(l, dict):
                loc_snippet = l.get("location_prompt_snippet") or f"{l.get('name', 'location')}, {l.get('environment', '')}, {l.get('lighting', '')}"
                locations.append(LocationProfile(
                    name=l.get("name", "Location"),
                    architecture=l.get("architecture", ""),
                    environment=l.get("environment", ""),
                    time_period=l.get("time_period", ""),
                    color_palette=l.get("color_palette", ""),
                    lighting=l.get("lighting", ""),
                    important_objects=l.get("important_objects", []),
        ...
        for idx, s in enumerate(data.get("scenes", [])):
            if isinstance(s, dict):
                scenes.append(VisualScene(
                    ...
                    image_prompt=s.get("image_prompt", f"{s.get('description', '')}, {req.visual_style} style, 8k resolution"),
                    negative_prompt=s.get("negative_prompt", prompt_engine.STYLE_NEGATIVE_PROMPTS.get(req.visual_style, "blurry, distorted, low quality"))
                ))
